...
if [ $INTERACTIVO == 1 ]; then
	COLOR_RESALTAR="\x1B[1m"
	COLOR_NORMAL="\x1B[0m"
	COLOR_ROJO="\x1B[91m"
	COLOR_VERDE="\x1B[92m"
	COLOR_CYAN="\x1B[96m"
	COLUMNAS=$(stty size | { read rows cols; echo $cols; } )
fi
...
function mk_temp() { TMPDIR=./ mktemp -t "tmp_pruebas_XXXXXX"; }
tmp_entrada_test=$(mk_temp)
tmp_salida_test=$(mk_temp)
tmp_salida=$(mk_temp)
tmp_valgrind=$(mk_temp)
tmp_stderr=$(mk_temp)
...
do
	...
	cat "$f" | grep '^<' | sed 's/^.//' >"$tmp_entrada_test"
	cat "$f" | grep '^>' | sed 's/^.//' >"$tmp_salida_test"

	if [ $CORRER_VALGRIND == 1 ]; then
		cmd_valgrind="valgrind --leak-check=full --show-reachable=yes -q --log-file=$tmp_valgrind"
	fi
	$cmd_valgrind "$PROGRAMA" <"$tmp_entrada_test" 2>"$tmp_stderr" | tr -d '\r' >"$tmp_salida"
