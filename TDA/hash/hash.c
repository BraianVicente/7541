...
// Pre: la tabla de hash debe existir, se debe indicar el tamanio de dicha ta-
// bla y se debe ingresar la funcion de destruccion de datos que guarda el
// hash. En caso de que no se deseen destruir, indicar NULL.
...
// Encuentra la posicion que ocupa la clave indicada o en su defecto una posi-
// cion vacia.
// Pre: la tabla de hash debe existir, se debe indicar el tamanio de dicha ta-
// bla, la clave que se desea buscar y ademas se debe indicar una semilla(i)
// desde donde se comienza a buscar.
...
static size_t buscar_sig_pos_ocupada(const hash_t* hash, size_t inicio){
	...
	if (inicio == hash->tamanio) return HASH_ITER_AL_FINAL;
	return inicio;
}
...
bool hash_iter_avanzar(hash_iter_t *iter){
	...
	if ( (iter->hash->tamanio) == (iter->actual) ) return false;
